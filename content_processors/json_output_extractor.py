...
Notes:
    一般的使用场景是:
        - 指定 LLM 进行结构化输出，需要从中提取结果。

    与 langchain 中的 OutputParser 的区别:
        - JsonOutputParser: 同样可以进行 json 数据的解析，同时支持 json 和 markdown-code-cell中 json 这 2 种。
            我的实现是 markdown-code-cell 版本。
            但是，langchain 中的实现较为简单，不能自定义相关功能。会默认提取第一个 json ，只能以最严格的 json 进行加载。
        - PydanticOutputParser: langchain 中基于 JsonOutputParser 的派生类，更强大，但是对于 pydantic 的 BaseModel 的定义有限。
        - GuardrailsOutputParser: 调用 LLM 进行修复 json ，但是需要额外导入包，并且不是原本的 LLM 。
        - RetryWithErrorOutputParser: 重新请求，但是请求方式过于简单，不是原本的请求方法。

    后续可能的改进:
        - 返回类型为 Union[BaseModel, None] ，如果需要获取 python object，直接以调用 model_dump 方法。但目前场景较复杂，未定解决方案。
"""
...
class JsonOutputExtractor:
    ...
    所有方法均返回 JSONReturnType 或者 None。None 无论是什么原因，都需要重新生成。
    ...
    def extract_json_from_str(
        ...
        Returns:
            Union[Union[dict, list], None]:
                - Union[dict, list]: 正常解析。输出可用于处理的 structured output 。
                - None: 解析失败。
    ...
    def check_dict_schema(
        ...
        """
        对于已经可以加载的 json 数据进行字段检验。

        静默检查，如果需要转换由外部工具实现。原因在于:
            - 暴露方法通用性。该工具类暴露的方法并不限制 dict 或 list ，如果获取的数据类，获取结果会需要额外不同外部逻辑。
            - 返回类型。返回类型只能标注为 BaseModel | None ，同样需要 cast 或者 assert 。
        ...
        Args:
            raw_dict_structured_data (dict): json 数据，在这一步被处理为 python 中的 dict 的数据。
            schema_pydantic_base_model (Type[BaseModel]): pydantic 定义的数据类。

        Returns:
            Union[dict, None]:
                - dict: 通过检测。但是不进行 dataclass 加载，而是由外部逻辑实现。
                - None: 未通过检测。
    ...
    def check_list_schema(
        ...
        """
        对于已经可以加载的 json 数据进行字段检验。对于 list 。

        我知道可以使用 pydantic 的 RootModel 简化这个方法实现。
        但是为了和 check_dict_schema 统一，以及避免额外转换逻辑。
        约定规范减少复杂性。

        注意: 这里的 pydantic-base-model 需要具体的定义形式。例如:
            ```python
            class ListToCheck(BaseModel):
                items: list[str] = Field(..., min_items=3, max_items=5)
            ```
            约定只有一个 `items` 字段，list 长度判断由 pydantic 中定义 Field 实现。
    ...
    def delete_last_json(
        ...
        """
        使用正则方法，从原始字符串中删除最后一个 markdown 的 json-cell 。

        Args:
            text (str): 原始文本。

        Returns:
            str: 删除最后后一个 markdown 的 json-cell 的原始文本。
        """
        pattern = r'```json(.*?)```'
